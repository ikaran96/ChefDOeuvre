@extends('pole.layout')
@section('content')
<a href="{{route('continues.index')}}"><i class="fas fa-arrow-left"></i></a>
<h2 class="add-title">Modifier {{ $continue -> name}}</h2>
<div class="form-wrapper">
<x-message/>

    <form method="POST" style="margin-left:auto;margin-right:auto" action="{{route('continues.update',$continue->id)}}">
        @csrf
        @method('patch')
        <div class="form-group row">
            <label for="name" class="col-sm-2 col-form-label">Nom *</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" name="name" id="name" placeholder="Nom de la formation" value="{{ $continue -> name}}">
            </div>
        </div>
        <div class="form-group row">
